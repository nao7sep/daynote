...
using DayNote.Views;
using Xunit;
using static DayNote.Views.MacMenuBar;
...
/// matches the layout, that the keys that type each shortcut reach its item, that DayNote's own
...
public sealed class MacMenuBarTests
{
    ...
    public void The_bar_speaks_the_interface_language_and_keeps_its_roles()
    {
        ...
        Assert.Equal(Localizer.T("nativeMenu.quit", ("app", "DayNote")), layout[0].Items.OfType<Item>().Last().Title);
    }

    [Fact]
    public void The_bar_is_the_app_menu_then_Edit_then_Window()
    {
        Assert.Equal(new[] { "DayNote", "Edit", "Window" }, Layout("DayNote").Select(menu => menu.Title));
    }

    [Fact]
    public void The_app_menu_names_DayNote_and_carries_About_Settings_Services_Hide_and_Quit()
    {
        Assert.Equal(
            new[]
            {
                "About DayNote", "—", "Settings…", "—", "Services", "—",
                "Hide DayNote", "Hide Others", "Show All", "—", "Quit DayNote",
            },
            Titles(Menu("DayNote")));
        Assert.Equal((",", Cmd), KeyOf(Menu("DayNote"), "Settings…"));
        Assert.Equal(("q", Cmd), KeyOf(Menu("DayNote"), "Quit DayNote"));
    }
    ...
    [Fact]
    public void Only_About_Settings_and_Quit_are_answered_by_DayNote()
    {
        var actions = Layout("DayNote").SelectMany(menu => menu.Items).OfType<Item>()
        ...
        Assert.Equal(
            ...
            Menu("DayNote").OfType<Item>().Select(item => item.Action)
    ...
    {
        var shortcuts = Layout("DayNote").SelectMany(menu => menu.Items).OfType<Item>()
    ...
    public void No_window_or_the_app_declares_an_Avalonia_NativeMenu()
    {
        ...
        var offenders = Directory.EnumerateFiles(Path.Combine(RepoRoot(), "src", "DayNote"), "*.*", SearchOption.AllDirectories)
    ...
    public void The_native_bar_matches_the_layout()
    {
        ...
        var bar = BuildBar("DayNote", target);
        var layout = Layout("DayNote");
    ...
    {
        LoadAppKit();
        var bar = BuildBar("DayNote", CreateAppActionTarget(UniqueClassName("Target")));
        foreach (var item in Layout("DayNote").SelectMany(menu => menu.Items).OfType<Item>().Where(item => item.Key.Length > 0))
        {
    ...
    [AvaloniaFact]
    public void DayNotes_own_window_shortcuts_get_past_the_native_bar_except_Settings()
    {
        ...
        {
            [Key.N] = "n", [Key.O] = "o", [Key.S] = "s", [Key.W] = "w", [Key.F] = "f", [Key.J] = "j",
            [Key.OemComma] = ",", [Key.OemQuestion] = "/",
        };
    ...
    {
        LoadAppKit();
        var bar = BuildBar("DayNote", CreateAppActionTarget(UniqueClassName("Target")), editShortcuts: false);
    ...
    public void The_app_target_answers_About_and_Settings_and_disables_them_when_told()
    {
        ...
        try
        {
            ...
            var aboutItem = NewItem(new Item("About DayNote", AboutAction), target);
            var quitItem = NewItem(new Item("Quit DayNote", QuitAction, "q"), target);
    ...
    private static Item?[] Menu(string title) => Layout("DayNote").Single(menu => menu.Title == title).Items;
    ...
    private static string UniqueClassName(string role) => $"DayNoteMenuBarTest{role}{Guid.NewGuid():N}";
    ...
    private static string RepoRoot([CallerFilePath] string path = "") =>
        // This file: <repo>/tests/DayNote.Tests/Views/MacMenuBarTests.cs

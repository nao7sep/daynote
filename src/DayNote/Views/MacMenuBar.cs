...
internal static class MacMenuBar
{
    ...
    // The actions DayNote answers itself; every other action is AppKit's own. Quit shuts down
    ...
    /// glued on, because where the name sits in "About DayNote" differs between languages.
    /// </summary>
    ...
    public static void Install(string appName, Action showAbout, Action showSettings, Func<bool> canShowAppDialogs)
    {
        ...
        try
        {
            ...
            // The log is not open yet while a startup failure is shown, nor in tests, hence Program.Log?.
            // here and below.
            var view = Class("AvnView");
            s_editShortcuts = CanTeachEditActions(view);
            if (!s_editShortcuts)
                Program.Log?.Warn("ui: Avalonia's macOS view cannot take the Edit actions; the Edit menu leaves its shortcuts to Avalonia");
        ...
        catch (Exception ex)
        {
            ...
            Program.Log?.Error("ui: the macOS menu bar could not be set", error: ex);
        }
    }
    ...
    private static void OnLanguageChanged()
    {
        ...
        catch (Exception ex)
        {
            ...
            Program.Log?.Error("ui: the macOS menu bar could not be rebuilt in the new language", error: ex);
        }
    }
    ...
    internal static void AddEditActions(IntPtr viewClass)
    {
        foreach (var action in EditActions)
        {
            if (!AddMethod(viewClass, action, s_editAction, ActionMethodTypes))
                Program.Log?.Info("ui: the macOS view already answers an Edit action", new { action });
        }

        if (!AddMethod(viewClass, "validateMenuItem:", s_validateEditItem, BoolMethodTypes))
            Program.Log?.Info("ui: the macOS view already validates menu items");
    }
    ...
    private static void OnAppAction(IntPtr self, IntPtr selector, IntPtr sender)
    {
        ...
        {
            Program.Log?.Error("ui: a macOS menu item failed", error: ex);
        }
    }
    ...
    private static byte ValidateAppItem(IntPtr self, IntPtr selector, IntPtr menuItem)
    {
        ...
        {
            Program.Log?.Error("ui: a macOS menu item could not be validated", error: ex);
            return 0;
        }
    }
    ...
    private static void OnEditAction(IntPtr view, IntPtr selector, IntPtr sender)
    {
        ...
        {
            Program.Log?.Error("ui: a macOS Edit menu item failed", error: ex);
        }
    }
    ...
    private static byte ValidateEditItem(IntPtr view, IntPtr selector, IntPtr menuItem)
    {
        ...
        {
            Program.Log?.Error("ui: a macOS Edit menu item could not be validated", error: ex);
            return 0;
        }
    }

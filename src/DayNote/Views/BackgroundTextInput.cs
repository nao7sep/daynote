    ...
    private static void Deliver(WindowBase window, TextInputEventArgs e)
    {
        try
        {
            ...
            if (!s_lastField.TryGetValue(window, out var last)
                ...
                || !field.Focus())
            {
                // The length only: what was typed is never logged. The log is not open in tests.
                Program.Log?.Info("ui: text reached a background window with no text field to take it", new { length = e.Text.Length });
                return;
            }

            field.RaiseEvent(new TextInputEventArgs { RoutedEvent = InputElement.TextInputEvent, Text = e.Text });
            e.Handled = true;
            Program.Log?.Info("ui: text that reached a background window went to its text field", new { length = e.Text.Length });
        }
        catch (Exception ex)
        {
            // A fault in this workaround must never break typing; the text stays where Avalonia put it.
            Program.Log?.Error("ui: background text could not be handed to its text field", error: ex);
        }
    }
}

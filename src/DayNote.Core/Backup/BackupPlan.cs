...
public static class BackupPlan
{
    ...
    /// granularity (a document may live on a USB stick); it costs nothing in missed edits because the
    /// recorded time is from a prior backup run, which any real edit moves well beyond two seconds past.
    /// </summary>
    ...
    private static bool IsChanged(BackupCandidate candidate, IReadOnlyDictionary<string, BackupIndexEntry> latest)
    {
        ...
        if (!DayNoteTime.TryParseIso(entry.LastWriteUtc, out var recorded))
        {
            return true;
        }

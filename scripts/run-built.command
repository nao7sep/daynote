...
pause_on_failure() {
  ...
  if [[ "$status" -ne 0 && ( "$status" -lt 128 || "$status" -gt 143 ) ]]; then
    echo
    echo "daynote run-built failed with exit code $status."
...
stop_owned_runtime dotnet "DayNote" "$REPO_DIR" "$REPO_DIR/src/DayNote/DayNote.csproj" "DayNote" "$APP_EXECUTABLE"
...
open -n "$APP_BUNDLE"
wait_for_owned_runtime dotnet "DayNote" "$REPO_DIR" "$REPO_DIR/src/DayNote/DayNote.csproj" "DayNote" "$APP_EXECUTABLE" 30
